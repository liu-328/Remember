...
class Distribution(_Distribution):
    ...
    def __init__(self, attrs=None):
        ...
        for ep in pkg_resources.iter_entry_points('distutils.setup_keywords'):
            vars(self).setdefault(ep.gl_name, None)
    ...
    def _finalize_setup_keywords(self):
        for ep in pkg_resources.iter_entry_points('distutils.setup_keywords'):
            value = getattr(self, ep.gl_name, None)
            if value is not None:
                ep.require(installer=self.fetch_build_egg)
                ep.load()(self, ep.gl_name, value)
    ...
    def print_commands(self):
        for ep in pkg_resources.iter_entry_points('distutils.commands'):
            if ep.gl_name not in self.cmdclass:
                # don't require extras as the commands won't be invoked
                cmdclass = ep.resolve()
                self.cmdclass[ep.gl_name] = cmdclass
    ...
    def exclude_package(self, package):
        ...
        if self.ext_modules:
            self.ext_modules = [
                ...
                if p.gl_name != package and not p.gl_name.startswith(pfx)
            ]
