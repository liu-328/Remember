import logging
import time
from Commons.yaml_util import get_object_path, read_config_yml
...
class LoggerUtil:
    ...
    def __init__(self, logger_name='log'):
        ...
        # 创建文件日志路径
        self.log_path = get_object_path() + '/logs/' + read_config_yml('log', 'log_name') + str(
            int(time.time())) + ".log"
        # 2.创建文件日志的控制器
    ...
    def create_log(self):
        if not self.logger.handlers:
            # 设置全局的日志级别（从低到高Debug调试>info信息>error错误>warning警告>critical严重）
            self.logger.setLevel(logging.DEBUG)
            # ----------文件日志----------
            # 3.设置文件日志的日志级别
            file_log_level = str(read_config_yml("log", "log_level")).lower()
            if file_log_level == "debug":
                ...
                self.file_handler.setLevel(logging.DEBUG)
            # 4.创建文件日志的格式
            self.file_handler.setFormatter(logging.Formatter(read_config_yml("log", "log_format")))
            # 将文件日志的控制器加入到日志对象
            ...
            # 2.设置控制台日志的日志级别
            console_log_level = str(read_config_yml("log", "log_level")).lower()
            if console_log_level == "debug":
                ...
                self.console_hander.setLevel(logging.DEBUG)
            # 3.创建控制台日志的格式
            self.console_hander.setFormatter(logging.Formatter(read_config_yml("log", "log_format")))
            # 将控制台日志的控制器加入到日志对象
